...
async def chat_completions(request: Request):
    ...
    # 保留原始非 system 消息（用于记忆提取和最终构建）
    original_non_system = [msg for msg in messages if msg.get("role") != "system"]
    ...
    # 重新构建 messages：system 在前，后跟原始非 system 消息
    new_messages = []
    if enhanced_prompt:
        new_messages.append({"role": "system", "content": enhanced_prompt})
    new_messages.extend(original_non_system)
    ...
    if is_stream:
        return StreamingResponse(
            stream_and_capture(headers, body, session_id, user_message, model, original_non_system),
...
async def stream_and_capture(headers, body, session_id, user_message, model, original_non_system):
    full_response = []
    line_buffer = ""
    async with httpx.AsyncClient(timeout=300) as client:
